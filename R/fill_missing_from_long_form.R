...
#'
#' @param indic_to_fill A long form Eurostat indicator.
#' that may contain implicitly missing values.
#' @importFrom dplyr mutate select group_by ungroup any_of
...
fill_missing_from_long_form <- function( indic_to_fill ) {
  ...
  fill_indicator <- function(x){
    tmp_fill <-  x %>%
      ...
      mutate ( method   = ifelse (is.na(.data$value), "missing", "actual"),
               value_type = ifelse (is.na(.data$value), "missing", "actual")
      )
  ...
  ##u <- nest_to_fill %>% tidyr::unnest(c("dataset_code", "description_at_source",
