...
small_pop_datacite_2 <- datacite_create (small_population_dataset,
                 dataset_code = "small_population_total",
                 Title = "Population on 1 January by age and sex",
                 Subject = "demography",
                 Creator = add_creators("Jane", "Doe", "Reprex"),
...
test_that("datacite_create work", {
  ...
  expect_equal(small_pop_datacite_2$Date,
               test_date_string)
})
...
relitems <-add_related_items (RelatedItem = "regions R package",
                   relatedItemType = "Software",
                   relationType = "isCompiledBy",
                   relatedItemIdentifier = regions_package,
                   format = "json",
                   related_items = item1 )

test_that("administrative entries work", {
  expect_equal(add_frequency("A", admin_format = "list"),
